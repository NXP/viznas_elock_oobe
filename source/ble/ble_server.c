...
attribute_t ble_gatt_user_attributes[] = {
    ...
    {HANDLE_UART_SERVICE_CHAR_CFG_DESC, 2, (void *)&hello_sensor_hostinfo.characteristic_client_configuration},
    {HANDLE_COMMISSIONING_SERVICE_CHAR_WIFI_VAL, 0, commissioning_char_wifi.val},
};

EventGroupHandle_t xEventGapSemaphore;
EventGroupHandle_t ble_cred_received_event;
QueueHandle_t ble_uart_recv_data_queue;
...
const uint8_t ble_gatt_database[] = {
    ...
    /* Declare UART Service with 128 byte UUID */
    PRIMARY_SERVICE_UUID128(HANDLE_UART_SERVICE, UUID_SERVICE_WIRELESS_UART),

    /* Declare characteristic use to write request */
    CHARACTERISTIC_UUID128_WRITABLE(HANDLE_UART_SERVICE_CHAR_WRITE,
                                    HANDLE_UART_SERVICE_CHAR_WRITE_VAL,
                                    UUID_UART_STREAM_TX,
                                    LEGATTDB_CHAR_PROP_WRITE_NO_RESPONSE,
                                    LEGATTDB_PERM_WRITABLE),

    /* Declare characteristic used to notify/indicate change */
    CHARACTERISTIC_UUID128(HANDLE_UART_SERVICE_CHAR_NOTIFY,
                           HANDLE_UART_SERVICE_CHAR_NOTIFY_VAL,
                           UUID_UART_STREAM_RX,
                           LEGATTDB_CHAR_PROP_NOTIFY,
                           LEGATTDB_PERM_READABLE),

    CHAR_DESCRIPTOR_UUID16(HANDLE_UART_SERVICE_CHAR_CFG_DESC, GATT_UUID_CHAR_CLIENT_CONFIG, LEGATTDB_PERM_READABLE),
...
static void hello_sensor_set_advertisement_data(void)
{
    ...
    uint8_t num_elem                        = 0;
    uint8_t uart_service_uuid[LEN_UUID_128] = {UUID_SERVICE_WIRELESS_UART};
    ...
    adv_elem[num_elem].len         = 4; // strlen((const char *)wiced_bt_cfg_settings.device_name);
    ...
    adv_elem[num_elem].advert_type = BTM_BLE_ADVERT_TYPE_128SRV_PARTIAL;
    adv_elem[num_elem].len         = LEN_UUID_128;
    adv_elem[num_elem].p_data      = uart_service_uuid;
    num_elem++;
...
static void hello_sensor_set_scan_response_data(void)
{
    ...
    uint8_t num_elem                                 = 0;
    uint8_t commissioning_service_uuid[LEN_UUID_128] = {UUID_COMMISSIONING_SERVICE};

    adv_elem[num_elem].advert_type = BTM_BLE_ADVERT_TYPE_128SRV_PARTIAL;
    adv_elem[num_elem].len         = LEN_UUID_128;
    adv_elem[num_elem].p_data      = commissioning_service_uuid;
    num_elem++;
                                                                             ...
                                                                             wiced_bt_gatt_write_t *p_data)
{
    ...
    p_data->p_val[p_data->val_len] = '\0';

    WPRINT_BT_APP_INFO(("write_handler:%s\r\n", p_data->p_val));

    switch (p_data->handle)
    {
        case HANDLE_UART_SERVICE_CHAR_WRITE_VAL:
            if ((xQueueSend(ble_uart_recv_data_queue, (char *)p_data->p_val, portMAX_DELAY)) != pdPASS)
            {
                WPRINT_BT_APP_INFO(("Cannot send data to queue\r\n"));
            }
            break;
    ...
    uint16_t conn_id, wiced_bt_gatt_exec_flag_t exec_flag)
{
    ...
    if (commissioning_char_wifi.credentials_received)
    {
        ...
        {
            /* check if the ble provisioning task exist*/
            if (ble_cred_received_event)
            {
...
int is_ble_gatt_connected(void)
{
    return is_connected;
}

int ble_send_notification(const uint8_t *data, int len)
{
    int retry                       = 5;
    ...
    if (!is_connected)
    {
        WPRINT_BT_APP_INFO(("ble haven't connected.\r\n"));
        return -1;
    }
    ...
    {
        res_gatt =
            wiced_bt_gatt_send_notification(hello_sensor_state.conn_id, HANDLE_UART_SERVICE_CHAR_NOTIFY_VAL, len, data);

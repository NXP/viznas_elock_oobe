...
__attribute__((section(".ocram_non_cacheable_data ,\"aw\", %nobits @")))
...
companion_message_t message_handle(struct netconn *conn, struct netbuf *buf_recv)
{
    ...
    PRINTF(("Received message from: %u.%u.%u.%u on port %d -> %s\r\n", ((u8_t *)&buf_recv->addr.addr)[0],
    ...
    if (parse_message(payload, MAX_MESSAGE_LEN, &message_type))
    {
        ...
        PRINTF(("Parsing the message failed\r\n"));
        goto clean;
    }
    ...
    {
        /* do not reply */
        PRINTF(("Will not reply to the message\r\n"));
        goto clean;
    }
...
companion_message_t apmode_message_handle(struct netconn *conn, struct netbuf *buf_recv)
{
    ...
    PRINTF("AP Received message from: %u.%u.%u.%u on port %d -> %s\r\n", ((u8_t *)&buf_recv->addr.addr)[0],
                  ((u8_t *)&buf_recv->addr.addr)[1], ((u8_t *)&buf_recv->addr.addr)[2],
                  ((u8_t *)&buf_recv->addr.addr)[3], buf_recv->port, payload);
    ...
    {
        /* invalid message, parsing failed or reply not needed */
        PRINTF(("AP Parsing the message failed\r\n"));
        goto clean;
    }
    ...
    PRINTF("AP Replying with %s\r\n", payload);
    //ip_addr_t ip_laptop[4] = {192, 168, 1, 102};
    net_err = netconn_send(conn, buf_send);
    ...
    if (net_err != ERR_OK)
    {
        ...
        PRINTF("AP sent message to: %u.%u.%u.%u on port %d -> %s\r\n", ((u8_t *)&buf_send->addr.addr)[0],
                      ((u8_t *)&buf_send->addr.addr)[1], ((u8_t *)&buf_send->addr.addr)[2],
                      ((u8_t *)&buf_send->addr.addr)[3], buf_send->port);
    }
    ...
    if (cJSON_GetArraySize(parsed_message) == 2)
    {
            ...
            (json_item2->valuestring != NULL))
        {
            ...
            *message_type = WIFI_CRED;
            ...
            PRINTF("Wifi credentials received: SSID '%s', Password '%s'\r\n", json_item1->valuestring,
            ...
            {
                PRINTF(("Cannot get wifi Password\r\n"));
                goto exit;
            }
            ...
            if (!credsave_status)
            {
                PRINTF("Wifi credentials saved in flash\r\n");
            }
            else
            {
                PRINTF("Failed to write wifi credentials in flash, error code %d\r\n", credsave_status);
            }
        }
    }
...
static uint8_t query_send_reply(companion_message_t message, companion_message_t *next_message)
{
    ...
    switch (g_expected_message)
    {
        ...
        default:
            /* should never happen */
            PRINTF("Strange value for expected companion message: %d\n", g_expected_message);
    }
...
static uint8_t generate_code_challenge(void)
{
    ...
    {
        PRINTF(("TRNG_GetRandomData failed\n"));
        return 1;
    }
    ...
    {
        PRINTF(("mbedtls_sha256_ret failed\n"));
        return 1;
    }

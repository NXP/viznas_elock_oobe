...
static EventGroupHandle_t s_UDPServerCloseEventGroup = NULL;
...
void udp_server_task(void *arg)
{
    ...
    s_UDPServerCloseEventGroup       = xEventGroupCreate();
#if LWIP_IPV6
    conn = netconn_new(NETCONN_UDP_IPV6);
    netconn_bind(conn, IP6_ADDR_ANY, 8888);
#else  /* LWIP_IPV6 */
    conn = netconn_new(NETCONN_UDP);
    /* set recv timeout in order to be more responsive */
    conn->recv_timeout = 500;
    err                = netconn_bind(conn, IP_ADDR_ANY, 8888);
    PRINTF("BIND ERR %d\r\n", err);
#endif /* LWIP_IPV6 */
    LWIP_ERROR("udpecho: invalid conn", (conn != NULL), return;);

    PRINTF("Starting UDP server, expecting companion app messages\r\n");

    if ((NULL == server_args) ||
        ((NETWORK_MODE_AP == server_args->network_mode) && (NULL == server_args->ap_close_event)))
    {
        PRINTF("Wrong task arguments\r\n");
        goto stop_task;
    }
    ...
    while (1)
    {
        ...
        if (err == ERR_OK)
        {
            ...
            {
                PRINTF(("WiFi credentials were received; Set the ap_close_event bit\r\n"));
            ...
            else if ((message_type == INVALID_MSG) && (NETWORK_MODE_STATION == server_args->network_mode))
            {
                ...
                {
                    PRINTF(("WiFi credentials were updated; Reset the board\r\n"));
        ...
        {
            if (xEventGroupWaitBits(s_UDPServerCloseEventGroup, 1, pdTRUE, pdTRUE, 0))
            {
                PRINTF(("WiFi credentials were provided in a different method. Close this task\r\n"));
                netbuf_delete(buf_recv);
                xEventGroupSetBits(server_args->ap_close_event, 1);
                break;
            }
...
stop_task:
    PRINTF(("Stop udp server and delete task\r\n"));

    /* clean resources */
    netconn_delete(conn);
    vEventGroupDelete(s_UDPServerCloseEventGroup);
    /* wake ais auth task */
...
void companion_app_status_task(void *arg)
{
    ...
    if (err != ERR_OK)
    {
        PRINTF(("udp_connect failed, error %d\r\n", err));
    }
    ...
    while (1)
    {
        ...
        {
            PRINTF(("strange status received: %d\r\n", appStatusEvents));
            continue;
        }
        ...
        {
            PRINTF(("status_message_create failed\r\n"));
            break;
        }
        ...
        if (err != ERR_OK)
        {
            PRINTF(("udp_send failed, error %d\r\n", err));
        }
...
int udp_server_close_event_set()
{
    if (s_UDPServerCloseEventGroup)
    {
        xEventGroupSetBits(s_UDPServerCloseEventGroup, 1);
    }
    else
    {
        return -1;
